// Maybe do not change this file and its html-master any more
...
// rotate data around Y axis. Is found in MyVectorLib.js
const rotY = ((deg) => { 
    const cd = Math.cos(Math.PI/180 * deg);
    const sd = Math.sin(Math.PI/180 * deg);
    vertices = vertices . map ( ([x,y,z]) => [
...
// or the zentral porojection to the x-y-plane
...
const createSvgFace = ((j) => {
        if (j==1){console.log("newly computed"+projType); console.log(viewLength)}
...
const visible = ((j) => {
   let midpt  = getCentroid(getFace(j)); // platonic case: faceNormal
   let result = midpt[2];
   if (projType == persp) {
        let midView = viewVector(midpt);
   		result = midpt[0]*midView[0] + midpt[1]*midView[1] + midpt[2]*midView[2];
   		//result = dotProdPts([midpt, viewVector(midpt)]);
   }
   return result; 
});
